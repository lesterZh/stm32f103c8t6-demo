...
// 在指定位置显示一个字符,包括部分字符
// x:0~127
// y:0~63, 0~7,y递增1向下移动8个像素
// mode:0,反白显示;1,正常显示
// size:选择字体 16/12
// 文字取模方式：按列取模，每次取一列的8个bit, 从上到下取，取模顺序是从低到高，即第一个点作为最低位。如*-------取为00000001
// 不足8位补够，取完第一列的8个点就取第二列的8个点, 8*16就是上下2部分，上半部分是8列，每列表示一个byte
...
// 显示汉字 16*16; 一个汉字32byte; id下标是16的倍数
void OLED_ShowCHinese(u8 x, u8 y, u8 id)
{
    u8 t, adder = 0;
    OLED_Set_Pos(x, y);
    for (t = 0; t < 16; t++)
    {
        OLED_WR_Byte(Hzk[2 * id][t], OLED_DATA);
        adder += 1;
    }
    OLED_Set_Pos(x, y + 1);
    for (t = 0; t < 16; t++)
    {
        OLED_WR_Byte(Hzk[2 * id + 1][t], OLED_DATA);
        adder += 1;
    }
}
